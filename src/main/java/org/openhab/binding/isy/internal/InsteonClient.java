...
public class InsteonClient extends ISYInsteonClient {
    ...
    private Logger logger = LoggerFactory.getLogger(InsteonClient.class);
    ...
    public void onModelChanged(final UDControl control, final Object action, final UDNode node) {
        ...
        logger.debug(String.format("onModelChanged(%s-%s,%s,%s)", control != null ? control.name : "",
                control != null ? control.label : "", action,
    ...
    public void onDeviceOffLine() {
        logger.info("ISY is now offline");
    }
    ...
    public void onNodeError(final UDNode node) {
        logger.info("Node {} reports error" + node.name);
    }
    ...
    public void onNewDeviceAnnounced(final UDProxyDevice device) {
        logger.info("Connected to ISY: " + device.getFriendlyName());
